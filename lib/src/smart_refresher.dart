...
/// [RefreshController] must not be null,Only one controller to one SmartScroll
///
...
/// See also:
///
/// * [RefreshConfiguration], A global configuration for all SmartScroll in subtrees
///
/// * [RefreshController], A controller controll header and footer  indicators state
class SmartScroll extends StatefulWidget {
  ...
  static SmartScroll? of(BuildContext? context) {
    return context!.findAncestorWidgetOfExactType<SmartScroll>();
  }

  static SmartScrollState? ofState(BuildContext? context) {
    return context!.findAncestorStateOfType<SmartScrollState>();
  }

  @override
  State<StatefulWidget> createState() {
    return SmartScrollState();
  }
}

class SmartScrollState extends State<SmartScroll> {
  ...
  @override
  void didUpdateWidget(SmartScroll oldWidget) {
...
/// See also:
///
/// * [SmartScroll],a widget help you attach refresh and load more function easily
class RefreshController {
  /// initialRefresh:When SmartScroll is init,it will call requestRefresh at once
  ///
  ...
  void _bindState(SmartScrollState state) {
    assert(_refresherState == null,
        "Don't use one refreshController to multiple SmartScroll,It will cause some unexpected bugs mostly in TabBarView");
  ...
  Future<void>? requestRefresh(
    ...
    if (indicatorElement == null || _refresherState == null) {
      return null;
    }
...
/// Controls how SmartScroll widgets behave in a subtree.the usage just like [ScrollConfiguration]
///
///
/// see also:
///
